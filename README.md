...
9. You came to know that your model is suffering from low bias and high variance. Which algorithm should you use to tackle it? Why?
   ...
   * The variance in a model is the amount that the estimate of the target function will change as a result to small
   fluctuations in the training set.
     * High variance can cause an algorithm to model the random noise in the training data, rather than the intended outputs
     (overfitting). -[Wikipedia](https://en.wikipedia.org/wiki/Bias%E2%80%93variance_tradeoff)
     * High-variance learning methods may be able to represent the training set well, but are at risk of overfitting to
     noisy or unrepresentative training data -[Wikipedia](https://en.wikipedia.org/wiki/Bias%E2%80%93variance_tradeoff)
   * The bias in a model is the underlying assumptions made about the form of the target function. For instance, Linear 
   regression assumes a linear distribution of underlying data. We could say that Linear Regression is highly biased
   toward a linear distribution.
      * High bias can cause an algorithm to miss the relevant relations between features and target outputs (underfitting) -[Wikipedia](https://en.wikipedia.org/wiki/Bias%E2%80%93variance_tradeoff). 
        * Consider the case of Linear Regression applied to a scatter plot with high variance. A single line will do
        poorly in capturing the variance, leading to textbook underfitting. 
      * Algorithms with high bias typically produce simpler models that don't tend to overfit but may underfit their
      training data -[Wikipedia](https://en.wikipedia.org/wiki/Bias%E2%80%93variance_tradeoff)
   * I would start by using a less complex model to give the model less freedom to overfit. I would then use cross 
   validation to ensure that this is the case. If the model is complex by design (as in neural networks), I would give 
   the network less neurons to learn with. In doing so, this reduces the number of hyperplanes that the network can draw
   in order to separate it's input into distinct classes. 
